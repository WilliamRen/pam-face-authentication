...
//cvWaitKey(1);
        ...
        if (ly<0)
            ly=0;
        if ((ly<0 || lx<0 || (ly+newHeight)>240 ||  (lx+newWidth)>320))
        {
            flag=0;
            messageIndex=3;
            return 0;
        ...
        leftEyePTemp.y=leftEyeP.y -(((leftEyeP1.y)*newWidth)/widthEyeWindow)+ temp.y;
        leftEyePTemp.x=leftEyeP.x -(((leftEyeP1.x)*newWidth)/widthEyeWindow)+ temp.x;
        leftEyeP1Temp.x=temp.x;
        leftEyeP1Temp.y=temp.y;
        rightEye.findPoint(rightEyeP1,&temp);
        rightEyePTemp.y=rightEyeP.y -(((rightEyeP1.y)*newWidth)/widthEyeWindow)+ temp.y;
        rightEyePTemp.x=rightEyeP.x -(((rightEyeP1.x)*newWidth)/widthEyeWindow)+ temp.x;
