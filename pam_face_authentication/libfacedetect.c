...
int preprocess(IplImage * img,CvPoint plefteye,CvPoint prighteye,IplImage * face)
{
    ...
    if (ratio>.5  && ratio<1.75 && reWidth>0 && reHeight>0)
    {
        ...
        p1LeftTop.x=plefteye.x-25;
        p1LeftTop.y=((plefteye.y+prighteye.y)/2)-40;
        p1RightTop.x=prighteye.x+25;
        p1RightTop.y=((plefteye.y+prighteye.y)/2)  +100;
        if (p1LeftTop.x>0 && p1LeftTop.y>0&& p1LeftTop.x<resizedImg->width && p1LeftTop.y<resizedImg->height && p1RightTop.x>0 && p1RightTop.y>0&&p1RightTop.x<resizedImg->width && p1RightTop.y<resizedImg->height && p1RightTop.y-p1LeftTop.y<resizedImg->height && p1RightTop.x-p1LeftTop.x <resizedImg->width)
        {
            if (CheckImageROI(resizedImg,p1LeftTop.x,p1LeftTop.y,p1RightTop.x-p1LeftTop.x,p1RightTop.y-p1LeftTop.y,2)!=-1)
            {
                ...
                /*
                                ...
                                int x =0;
                                int y =0;
                                CV_INIT_PIXEL_POS(pos_src,(unsigned char *) face->imageData,face->widthStep,cvGetSize(face),
                                                  x,y,
                                                  face->origin);
                                uchar * ptr_src;
                                double side1,side2;
                                ...
                                {
                                    for ( x=0; x<(face->width/2); x++)
                                    {

                                        ptr_src = CV_MOVE_TO(pos_src,x,y,3);
                                        side1+= ptr_src[0];
                                    }
                                    for ( x=(face->width/2); x<(face->width); x++)
                                    {
                                        ptr_src = CV_MOVE_TO(pos_src,x,y,3);
                                        side2+= ptr_src[0];
                                ...
                                if (side1>side2)
                                {
                                    IplImage* faceTemp = cvCreateImage( cvSize(face->width/2,face->height),8,1);
                                    cvSetImageROI(face,cvRect(face->width/2,0,face->width/2,face->height));
                                    cvResize(face,faceTemp,CV_INTER_LINEAR);
                                    cvResetImageROI(face);
                                    cvFlip(faceTemp,NULL,1);
                                    cvSetImageROI(face,cvRect(0,0,face->width/2,face->height));
                                    cvResize(faceTemp,face,CV_INTER_LINEAR);
                                    cvResetImageROI(face);
                                    cvReleaseImage( &faceTemp );
                                }
                                else
                                {
                ...
                */
                ...
                // ptr_src[0] = 255;
                // else
                //          if ((sqrt(pow((x-((face->width)/2)),2) +pow((y-((face->height)/2)),2) )>((face->height)/2)) &&  (y>(face->height/2)))
                //         {  ptr_src[0] = 0;
//
                //        }
...
int faceDetect( IplImage* img,CvPoint *pLeftEye,CvPoint *pRightEye,CvMemStorage* storageFace,CvMemStorage* storageEyeLeft,CvMemStorage* storageEyeRight,CvMemStorage* storageNose,CvHaarClassifierCascade* cascadeFace,CvHaarClassifierCascade* cascadeEyeLeft,CvHaarClassifierCascade* cascadeEyeRight,CvHaarClassifierCascade* cascadeNose)
{
    ...
    int i,i1;
    // IplImage* imgMask = cvCreateImage( cvSize(img->width,img->height), 8,3 );
    //  IplImage* gray2 = cvCreateImage( cvSize(img->width,img->height), 8,3 );
...
//printf(" no error ALL");
        ...
        else if (dtdRightEyeOnce==1)
        {
            ...
            if (ratioWidth>0 &&  ratioHeight>0 &&ratioWidth<4&&  ratioHeight<4&&(eyeRightPImage->width-(eyeTemplateRight->width)*ratioWidth)>=0 && ((eyeRightPImage->height)-(eyeTemplateRight->height)*ratioHeight)>=0 &&ratioWidth>0 && ratioHeight>0 )
            {
                ...
                {
                    //printf(" 4 %d %d %d %d \n",(minloc.x),(minloc.y),(eyeTemplateRightResized->width),(eyeTemplateRightResized->height));
    ...
    //cvFillConvexPoly(img, pts,4,CV_RGB(0,140,0),CV_AA,0 );
    //CvFont myFont;
    //cvInitFont(&myFont,CV_FONT_HERSHEY_DUPLEX, 1.0f,1.0f,0,1,CV_AA);
    cvReleaseImage( &gray );
    cvReleaseImage( &small_img );

    if (dtdLeftEye==1 && dtdRightEye==1)
    {
        //cvPutText(img,"DETECTED!", cvPoint(80,230),&myFont, CV_RGB(255,255,255));
        cvRectangle(img,p1,p2,colors[3],3,4,0);
    ...
    {
        //cvPutText(img,"NO DETECTION!", cvPoint(45,230),&myFont, CV_RGB(255,255,255));
        return 0;
    }
}
/*
...
intialize();
       ...
       buttonValue=cvWaitKey( 1000 );

 cvNamedWindow( "Face Tracker", 1 );

 p1Nose=cvPoint(0,0);
p2Nose=cvPoint(1,1);
p1RightEye=cvPoint(2,2);
p1LeftEye=cvPoint(2,2);
...
CvPoint p1,p2;
allocateMemory();
 if(frame_copy->width>0)
value=faceDetect(frame_copy,&p1,&p2,storageFace,storageEyeLeft,storageEyeRight, storageNose, cascadeFace,cascadeEyeLeft,cascadeEyeRight,cascadeNose);
cvShowImage( "Face Tracker", frame_copy );
      if( cvWaitKey( 4 ) >= 0 )
                break;
//printf( "2Pease Look at the Center of the Screen for the next five seconds. \n");
