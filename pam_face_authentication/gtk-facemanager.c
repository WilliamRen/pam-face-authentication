...
on_btnSSave_clicked  (GtkButton *button,gpointer user_data)
{
    ...
    if (file1 = fopen("/lib/pamface/facemanager/face.key", "w"))
    {
...
on_btnAbout_clicked  (GtkButton *button,gpointer user_data)
{
    /*
    GtkWidget *abtProj;
    abtProj = GTK_WIDGET (gtk_builder_get_object (builder, "abtProj"));
    gtk_dialog_run(abtProj);
    gtk_widget_hide(abtProj);
    */
    ...
    gtk_about_dialog_set_copyright(GTK_ABOUT_DIALOG(dialog),"Work done during Google Summer of Code 2008 for openSUSE by Rohan Anil and mentored by Alex Lau");
...
void  on_changed(GtkWidget *widget, gpointer label)
{
    ...
    if (gtk_tree_selection_get_selected(GTK_TREE_SELECTION(widget), &model, &iter))
    {
     ...
     //   lblUserName = GTK_WIDGET (gtk_builder_get_object (builder, "lblUserName"));
      //  gtk_label_set_text(GTK_LABEL(lblUserName), value);
   //     hideShowSelectArea(1);
        loadFaceImage(value);
        g_free(value);
    }
    else
...
*/
/*
void hideShowSelectArea(int t)
{

    GtkWidget *imgCapturedFace;
    GtkWidget *btnCaptureFace;
    imgCapturedFace = GTK_WIDGET (gtk_builder_get_object (builder, "imgCapturedFace"));
    btnCaptureFace = GTK_WIDGET (gtk_builder_get_object (builder, "btnCaptureFace"));
    if (t==1)
    {
        GdkPixbuf *pixbuf = NULL;
        pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE,8,320,240);
        gtk_image_set_from_pixbuf(imgCapturedFace, pixbuf);
        gtk_widget_show (btnCaptureFace);
        gtk_widget_show (imgCapturedFace);
    }
    if (t==0)
    {
        gtk_widget_hide (btnCaptureFace);
        gtk_widget_hide (imgCapturedFace);
    }
}
...
static gboolean time_handler(GtkWidget *widget)
{
    ...
    /*  GtkWidget *imgWebcam;
      ...
      loadCVPIXBUF(imgWebcam,frame_copy);
    */
...
{
    handleFaceDetect = dlopen(FACE_DETECT_LIBRARY, RTLD_LAZY);
    faceDetect = dlsym(handleFaceDetect, "faceDetect");
    preprocess = dlsym(handleFaceDetect, "preprocess");
    // dlerror();
    handleFaceConfigure = dlopen(FACE_CONFIGURE_LIBRARY, RTLD_NOW);
    ...
    gtk_builder_add_from_file (builder, "/lib/pamface/gtk-facemanager.xml", NULL);
